...
sesca_plot <- function(
    sescaPyClass, # contains the SESCA results
    plot_width,
    plot_height,
    plot_type,
    axis_size,
  ...
  # Activate average_ensemble if there is more than one predicted spectrum
  average_ensemble <- average_ensemble && (length(spectraNames_filt) > 1)
...
plot_heatmap_bayes_posterior <- function(df,i=1,j=2,plot_width=12,plot_height=12,plot_type='svg',plot_axis_size=18) {
    ...
    time <- strsplit(as.character(Sys.time())," ")[[1]][1]

    fig <- configFig(
        fig,paste0("sesca_heatmap_bayes_posterior_",time),
        plot_type,plot_width,plot_height)
